...
| 保全 | `18`・`19`（突発依頼2件と治工具メンテの実績。消耗品を在庫から引き落とす）。計画保全 SMP-MT-003 は指示のまま | SMP-MT-003 の **保全実績**、**消耗材モニタリング** で消費を確かめる |
| 治工具・校正 | `20`〜`22`（TL-01 は寿命の80%を超えて警告、TL-03 は払い出したまま使用中、期限切れだった DV-03 を校正） | 治工具管理の **寿命の警告**、TL-03 の **返却** |
| 在庫・物流 | `23`〜`27`（分割・移動・調整・状態変更・廃棄・返品・払出戻し、搬送2件、ピッキング2件、棚卸2件。SMP-ST-001 は確定まで） | 指示のまま残した搬送・出荷ピッキングの **実行**、SMP-ST-002 の **実棚入力と確定**、**倉庫業務進捗** |
| 不適合・サンプル | `28`・`29`（受入検査の不合格で起票された不適合を廃棄→承認まで、特採を指示した SMP-NC-001、保管サンプル2件） | SMP-NC-001 の **特採承認**、保管サンプルの **保管期限** の確認 |
