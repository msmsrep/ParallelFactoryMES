...
CSVに含まれない操作（完了承認・出荷など）は、<a href="#try">最後の章</a>にまとめてあります。</p>
</div>
...
出荷番号・判定書番号は採番した日で決まるので、写真の番号（SH20260922-0001 など）は取り込んだ日によって変わります。
...
|---|---|---|
| マスタ | `samples/master-csv` の19ファイル（作業区〜ユーザーのスキル） | ― |
| 受入 | `01_receiving.csv`（部材7ロット・保全部品1ロット） | ― |
| 製造指図 | `02_manufacturing-orders.csv`（SMP-SF-001・SMP-FG-001 は展開済み、SMP-FG-002 は承認済み、SMP-FG-003 は未承認） | SMP-FG-002 の **工程展開** と **差立** |
| 実行 | `03`〜`09`（段取り・点検・投入・出来高・製造条件） | 各作業指示の **製造完了承認**（pm001） |
| 検査 | `10_inspections.csv`（3件。判定まで） | 各検査の **検査承認** |
| 出荷 | ― | **出荷指示**（サンプル商事株式会社・FG-1000 ×5）→ **出荷判定**（可）→ **承認** → **出荷実行**（FG1000-S001 から5台） |
