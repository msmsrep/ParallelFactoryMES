...
| 受入 | `01_receiving.csv`（部材10ロット・保全部品1ロット。期限切れ・期限間近のロットを含む） | 在庫照会の **期限アラート** で期限切れ・期限間近のロットを確かめる |
...
| 検査 | `10_inspections.csv`（4件。判定まで。追加入荷の受入検査は不合格） | 各検査の **検査承認**、不合格で起票された **不適合** の対応 |
