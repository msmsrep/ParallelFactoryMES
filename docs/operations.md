...
保持期間を過ぎた記録を整理する運用向けの操作です。**画面はありません**。
下記のバックアップを取得したうえで、システム管理者のトークンで次のAPIを呼びます。
...
`to` に指定した日を含めて、それ以前の記録が対象です。
`dryRun=true` のあいだは**件数を返すだけでDBには触りません**。確認してから `dryRun` を外して実行します。
