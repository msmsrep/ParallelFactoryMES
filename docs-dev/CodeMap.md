...
| DB・DbContext・スキーマ | `Infra/MesAppDbContext.cs`（`DbSet` と enum の文字列変換）<br>`Infra/Configurations/<領域>Configurations.cs`（エンティティごとの索引・最大長・関連。System / Master / Production / Execution / Inventory / Quality / Maintenance）<br>`Infra/DependencyInjection.cs`（`UseProvider` でプロバイダー切替、起動時 `MigrateAsync`）<br>`Infra/DatabaseOptions.cs`・`Infra/DatabaseProviders.cs`<br>`Infra/ProviderModelConventions.cs`（プロバイダー差の吸収）<br>`Tests/DatabaseProviderTests.cs` | Spec.md 4章。既定 SQLite、設定で PostgreSQL / SQL Server。プロバイダー差の吸収（PGの `DateTimeOffset` UTC化・SQLite以外の `decimal(18,6)`・SQL Server の `ClientSetNull`）は `ProviderModelConventions` に集める。マイグレーションは `Infra/Migrations/`・`src/MesApp.Migrations.PostgreSql`・`.SqlServer` の3か所で、**読まない**。スキーマ変更は3つとも同名で追加（欠けると `DatabaseProviderTests` が落ちる） |
