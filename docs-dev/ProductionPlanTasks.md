...
| ID | 内容 | 想定層 | 状態 |
|:--|:--|:--|:--|
| PLAN-01 | 生産計画のデータモデルと登録API | L1 | 未着手 |
| PLAN-02 | 工程別の予実集計API | L1 | 未着手 |
| PLAN-03 | 生産計画の画面（計画登録タブ・予実タブ） | L2 | 未着手 |
| PLAN-04 | 生産計画のCSV取込・出力 | L1 | 未着手 |
...
- 受入条件(DoD):
  ...
  - 作業区の絞り込みは、実績側では `WorkOrder.WorkCenterId`（展開時点の値）で行い、上位の段を指定したら配下へ展開する
