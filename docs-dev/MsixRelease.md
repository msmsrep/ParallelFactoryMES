...
- ウィンドウ上端に初回ログインの案内（ユーザー名 `admin` と、**インストールごとに自動生成されたパスワード**）が表示されている
- その資格情報でログインでき、パスワード変更を求められる
- パスワード変更を済ませると、再起動を待たずに上端の案内が消える
- 変更前に再起動すると、案内のパスワードが前回と違う値になっている（生成値は保存していないため）
- マスタ登録・指図・実績入力が一通り動く
...
6. **申請オプション → 認定メモ**：審査員が動かせるよう、以下を必ず書く
   ...
   初回起動時に初期管理者が自動作成されます。
     ユーザー名: admin
     パスワード: 起動後、ウィンドウ上端の黄色い帯に表示されます
                 （インストールごとに自動生成されるため固定値ではありません）
