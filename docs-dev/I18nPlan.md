...
| I18N-09 | CSV：列の表示名・説明・取込エラー（列キーは英語のまま変えない） | **対応済**（Spec.md 改訂108。`MasterCsvKinds`・`ActualCsvKinds` は原文のまま持ち、`kinds` API が `CsvImport.Localize` で訳して返す。参照先・一意性の名前（`Reference`・`CheckUnique` に渡す「品目コード」等）は受け取る側で訳す。`CsvEnumLabels`（日本語の入力値）は訳さない。API の英訳の `{0}` の数は `AuthTests` で検査する） |
| I18N-10 | 仕上げ：`scripts/I18nCheck.ps1`（`en.resx` に無いキー・`L[]` を通っていない日本語リテラルの一覧）、CLAUDE.md の規約改訂 | **対応済**（Spec.md 改訂109。About・監査ログ・パスワード変更・初期セットアップ・実績CSV取込・権限なし／404 の画面も訳した。ログイン・パスワード変更の失敗時の既定文言は `AuthService` の引数にして画面側で訳す。API の日本語リテラルは保存される文字列と見分けられないため I18nCheck の対象外。`-ShowUnused` で使われていない英訳の候補を出す（変数で渡すキーがあるので参考扱い）） |
